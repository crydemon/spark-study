...
object Examples {
  ...
  def sum(ints: IndexedSeq[Double]): Par[Double] = // `IndexedSeq` is a superclass of random-access sequences like `Vector` in the standard library. Unlike lists, these sequences provide an efficient `splitAt` method for dividing them into two parts at a particular index.
    if (ints.size <= 1)
  ...
  def sum2(ints: IndexedSeq[Double]): Double =
    if (ints.size <= 1)
  ...
  def main(args: Array[String]): Unit = {
    ...
    val ins = 0.to(10).map(_ * 1.0)
    var start = LocalDateTime.now()

    start = LocalDateTime.now()
    sum2(ins)
    println(LocalDateTime.now().getNano - start.getNano)

    val svc = Executors.newFixedThreadPool(6)
    val p = sum(ins)
    Par.run(svc)(p).get()
    println(LocalDateTime.now().getNano - start.getNano)
