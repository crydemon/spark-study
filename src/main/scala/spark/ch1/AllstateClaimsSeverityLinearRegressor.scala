...
import org.apache.spark.sql.{Row, SaveMode, functions}
...
object AllstateClaimsSeverityLinearRegressor {
  def main(args: Array[String]) {
    ...
    val pipeline = new Pipeline().setStages(Preproessing.stringIndexerStages :+ Preproessing.oneHot :+ Preproessing.assembler :+ model)
    ...
    cvModel.transform(Preproessing.testData)
      ...
      .withColumn("id", functions.col("id").cast("int"))
      .withColumnRenamed("prediction", "loss")
      .coalesce(1)
      .write
      .option("header", "true")
      .mode(SaveMode.Overwrite)
      .csv("output/result_LR.csv")
